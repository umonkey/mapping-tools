...
export MAPILLARY_TOOLS_MAX_SEQUENCE_LENGTH=99999

if [ -z "$1" ]; then
    echo "Usage: $0 image_folder_path"
    exit 1
fi

if [ ! -d "$1" ]; then
    echo "This script uploads JPEG files from a folder to Mapillary."
    echo "Please run the app first to generate the files."
    exit 1
fi

mapillary_tools process_and_upload "$1" \
